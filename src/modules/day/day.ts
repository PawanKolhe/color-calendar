...
export function selectDayInitial(this: Calendar, setDate?: boolean) {
  if (setDate) {
    ...
    if (this.daysIn_CurrentMonth[idx]) {
      this.daysIn_CurrentMonth[idx]!.selected = true;
    }
  } else {
...
export function handleCalendarDayClick(this: Calendar, e: any) {
  ...
  if (day) {
    ...
    if (this.daysIn_CurrentMonth[dayNum - 1]) {
...
export function renderDays(this: Calendar) {
  ...
  let isTodayMonth =
    this.today.getMonth() === this.currentDate.getMonth() && isTodayYear;
